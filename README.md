...
to make camera's live :
ssh aesthetics-lab@192.168.88.215   (server's ip)
cd /home/aesthetics-lab/50
source .venv/bin/activate

export RTSP_URL="rtsp://admin:Admin@123@115.186.118. 100:554/cam/realmonitor?channel=1&subtype=1" (camera's's ip)
export CONF_THRESHOLD="0.4"
export REID="1"
export REID_SIM="0.62"
export PORT="8184"

gunicorn server:app --workers 1 --threads 2 --timeout 600 --bind 0.0.0.0:${PORT}

 Check and open Ubuntu firewall (ufw)
In your SSH session on the Ubuntu server:
sudo ufw status
If it says Status: active, run:
sudo ufw allow 8183/tcpsudo ufw allow 8184/tcpsudo ufw allow 8185/tcp